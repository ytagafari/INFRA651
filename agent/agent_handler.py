...
from agent.credibility_assessor import CredibilityResult, assess_credibility
from agent.document_selector import SelectedDocumentSet, select_documents
from agent.client_profile import ClientProfile, build_profile, company_slug, load_profile, save_profile
...
class AgentRunResult:
    ...
    selected_documents: SelectedDocumentSet | None = None
...
class OpportunityAgent:
    ...
    def handle(self, payload: OpportunityNotePayload) -> AgentRunResult:
        payload.validate()
        credibility = assess_credibility(payload.company, payload.discussion_topics, payload.notes)
        retrieval = self.kb.retrieve(payload.discussion_topics, payload.notes)
        selection = select_documents(retrieval, payload.discussion_topics, payload.notes)
        existing_profile = load_profile(company_slug(payload.company))
        client_profile = build_profile(payload, credibility, retrieval, selection, existing_profile)
        profile_path = save_profile(client_profile)
        retrieval.documents = selection.documents
        prompt = build_agent_prompt(payload, retrieval)
        attachments, _attach_notes = build_attachments_from_documents(selection.documents)
        ...
        artifact = {
            ...
            "document_selection": selection.to_dict(),
            "client_profile": client_profile.to_dict(),
            "client_profile_path": str(profile_path),
            "retrieval": {
